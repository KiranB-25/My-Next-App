...
const PortfolioNavbar = () => {
  ...
  return (
    ...
    >
      <div className='max-w-8xl mx-auto py-0 px-3 xs:px-4 sm:px-6 lg:px-8'>
        <div className='flex items-center justify-between h-14 xs:h-16 sm:h-18 md:h-20'>
          ...
          <div className='flex-shrink-0 group'>
            <div className='relative'>
              ...
              <div className='relative bg-black rounded-lg px-2 xs:px-3 sm:px-4 py-1 xs:py-2'>
                <span className='text-sm xs:text-base sm:text-xl md:text-2xl font-bold bg-gradient-to-r from-white via-purple-400 to-blue-400 bg-clip-text text-transparent'>
                  @CodeLand
                </span>
              </div>
            </div>
          ...
          {/* Desktop Navigation - Now responsive for medium to large screens */}
            ...
            <div className='ml-4 xl:ml-10 flex items-baseline space-x-1'>
              {navItems.map((item, index) => (
                <a
                  key={item.name}
                  href={item.href}
                  className='relative px-2 xl:px-4 py-2 text-sm xl:text-base text-gray-300 hover:text-white transition-all duration-300 group'
            ...
            <div className='ml-4 flex items-baseline space-x-1'>
              {navItems.slice(0, 4).map((item, index) => (
                <a
                  key={item.name}
                  href={item.href}
                  className='relative px-2 py-2 text-sm text-gray-300 hover:text-white transition-all duration-300 group'
                  style={{ animationDelay: `${index * 100}ms` }}
                >
                  <span className='relative z-10'>{item.name}</span>
                  <div className='absolute inset-0 bg-gradient-to-r from-pink-500/20 via-purple-500/20 to-blue-500/20 rounded-lg opacity-0 group-hover:opacity-100 transition-all duration-300 scale-95 group-hover:scale-100'></div>
              ...
              <div className='relative group'>
                <button className='px-2 py-2 text-sm text-gray-300 hover:text-white transition-all duration-300'>
                  More
                </button>
                <div className='absolute top-full right-0 mt-2 w-32 bg-black/90 backdrop-blur-lg rounded-lg shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300'>
                  {navItems.slice(4).map((item) => (
                    <a
                      key={item.name}
                      href={item.href}
                      className='block px-3 py-2 text-sm text-gray-300 hover:text-white transition-colors duration-300 hover:bg-gradient-to-r hover:from-pink-500/10 hover:via-purple-500/10 hover:to-blue-500/10 first:rounded-t-lg last:rounded-b-lg'
                    >
                      {item.name}
                    </a>
                  ))}
          ...
          {/* Social Links & CTA - Desktop and Large Tablets */}
          <div className='hidden md:flex items-center space-x-2 lg:space-x-4'>
            ...
            >
              ...
              <div className='relative px-3 lg:px-6 py-1.5 lg:py-2 bg-black rounded-lg leading-none flex items-center space-x-1 lg:space-x-2'>
                <span className='text-xs lg:text-sm text-white group-hover:text-gray-100 transition duration-200'>
                  Resume
                </span>
                <DownloadIcon className='w-3 h-3 lg:w-4 lg:h-4 text-white' />
              </div>
            </button>
          </div>

          {/* Small tablets - Show only essential items */}
          ...
          <div className='md:hidden flex items-center space-x-2'>
            {/* Essential social link for mobile */}
      ...
      {/* Mobile Navigation Menu - Enhanced for all mobile sizes */}
      <div
      ...
      >
        <div className='bg-black/95 backdrop-blur-lg border-t border-gray-800'>
          <div className='px-3 xs:px-4 pt-2 pb-3 space-y-1 max-h-96 overflow-y-auto'>
            {navItems.map((item, index) => (
              <a
                key={item.name}
                href={item.href}
                onClick={() => setIsOpen(false)}
                className='block px-4 py-3 text-sm xs:text-base text-gray-300 hover:text-white transition-all duration-300 rounded-lg hover:bg-gradient-to-r hover:from-pink-500/10 hover:via-purple-500/10 hover:to-blue-500/10'
            ...
            {/* Mobile Social Links - Better spacing for touch */}
            <div className='flex items-center justify-center space-x-6 xs:space-x-8 pt-4 pb-2 border-t border-gray-800 mt-4'>
              <a
                href='https://github.com/KiranB-25'
                target='_blank'
                rel='noopener noreferrer'
                className='text-gray-400 hover:text-white transition-all duration-300 transform hover:scale-110 p-2'
              >
                <GithubIcon />
              </a>
              <a
            ...
            {/* Mobile CTA Button - Full width for better touch */}
            <div className='px-2 pt-2 pb-4'>
              <button
                ...
                className='w-full relative group min-h-[48px]'
              >
                ...
                <div className='relative w-full px-6 py-3 bg-black rounded-lg flex items-center justify-center space-x-2'>
                  <span className='text-sm xs:text-base text-white'>Download Resume</span>
